/**
 * EmojiPicker - Interactive emoji selection component
 * Beautiful, categorized emoji picker for personalization
 */
...
const CATEGORY_ICONS: Record<EmojiCategory, string> = {
  smileys: 'emoticon-happy-outline',
  animals: 'paw',
  food: 'food-apple',
  activities: 'soccer',
  travel: 'airplane',
...
export default function EmojiPicker({
  ...
  const [selectedCategory, setSelectedCategory] = useState<EmojiCategory>(categories[0]);
  const [searchQuery, setSearchQuery] = useState('');
  const [recentEmojis, setRecentEmojis] = useState<string[]>([]);

  const filteredData = useMemo(() => {
    return EMOJI_DATA.filter(data => categories.includes(data.category));
  }, [categories]);

  const currentEmojis = useMemo(() => {
    const categoryData = filteredData.find(d => d.category === selectedCategory);
    if (!categoryData) return [];
    
    if (searchQuery.trim()) {
      // Simple search - filter emojis
  ...
  return (
    <Animated.View entering={FadeIn} style={[styles.container, style]}>
        ...
        {filteredData.map((data) => (
          <TouchableOpacity
            ...
            style={[
              styles.categoryTab,
              {
                backgroundColor: selectedCategory === data.category
          ...
          >
            <Icon
              name={CATEGORY_ICONS[data.category]}
              size={24}
              color={
            ...
            <Text
              style={[
                ...
                {
                  color: selectedCategory === data.category
                    ? '#fff'
                    : theme.colors.textSecondary,
                },
              ]}
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <Animated.View entering={FadeInDown} style={styles.emojiGrid}>
